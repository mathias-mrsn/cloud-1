...
if [ "$ENABLE_LOCAL_STACK" = "true" ]; then
        if [ -n "$WORDPRESS_ADMIN_SECRET_ARN" ]; then
...
elif [ -n "$WORDPRESS_ADMIN_SECRET_ARN" ] && [ -n "$PREFIX" ]; then
        WORDPRESS_ADMIN_SECRET_JSON="$(aws secretsmanager get-secret-value --secret-id "$WORDPRESS_ADMIN_SECRET_ARN" --query SecretString --output text)"
...
while [ ! -f "${WP_ROOT}/wp-settings.php" ] || [ ! -f "$WP_CONFIG_PATH" ] || [ ! -f "${WP_ROOT}/healthz.php" ] || ! /usr/local/bin/wp core is-installed --path="${WP_ROOT}" --allow-root >/dev/null 2>&1; do
